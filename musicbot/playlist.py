...
class Playlist(EventEmitter, Serializable):
    ...
    async def add_youtube_url_entry(self, song_url, **meta):
        """
            Validates and adds a youtube song_url to be played. This does not start the download of the song.
    ...
    async def async_process_youtube_playlist(self, playlist_url, **meta):
        ...
        for entry_data in info['entries']:
            if entry_data:
                ...
                try:
                    entry, elen = await self.add_youtube_url_entry(song_url, **meta)
