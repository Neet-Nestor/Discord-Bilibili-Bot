...
class MusicBot(discord.Client):
    def __init__(self, config_file=None, perms_file=None, aliases_file=None):
        ...
        self.str = JsonFile(self.config.i18n_file)
        ...
        for guild, channel in channel_map.items():
            ...
            if channel and isinstance(channel, discord.VoiceChannel):
                ...
                try:
                    ...
                    log.debug("Error joining {0.guild.name}/{0.name}\nError:{1}".format(channel, e), exc_info=True)
                    log.error("Failed to join {0.guild.name}/{0.name}\nError:{1}".format(channel, e))
    ...
    async def on_player_play(self, player, entry):
        ...
        if author not in player.voice_client.channel.members and author_perms.skip_when_absent:
            newmsg = 'Skipping next song in `%s`: `%s` added by `%s` as queuer not in voice' % (
                player.voice_client.channel.name, title, entry.meta['author'].name)
            player.skip()
        elif self.config.now_playing_mentions:
            newmsg = '%s - your song `%s` is now playing in `%s`!' % (
                entry.meta['author'].mention, title, player.voice_client.channel.name)
        else:
...
#######################################################################################################################
    ...
    def _gen_embed(self):
        ...
        e = discord.Embed()
        e.colour = 7506394
        e.set_footer(text='Neet-Nestor/Discord-Bilibili-Bot ({})'.format(BOTVERSION), icon_url='https://i.imgur.com/gFHBoZA.png')
        e.set_author(name=self.user.name, url='https://github.com/Neet-Nestor/Discord-Bilibili-Bot', icon_url=self.user.avatar_url)
        return e
    ...
    async def cmd_blacklist(self, message, user_mentions, option, something):
        ...
        if option in ['+', 'add']:
            ...
            if self.blacklist.isdisjoint(user.id for user in user_mentions):
                return Response(self.str.get('cmd-blacklist-none', 'None of those users are in the blacklist.'), reply=True)
    ...
    async def cmd_play(self, message, player, channel, author, permissions, leftover_args, song_url):
        ...
        # Extract av or BV info (bilibiliID)
        ...
        matchPlaylist = re.search(playlistRegex, song_url)
        groups = matchPlaylist.groups() if matchPlaylist is not None else []
        ...
        async with self.aiolocks[_func_() + ':' + str(author.id)]:
            ...
            # If it's an entry
                ...
                entry, position = await player.playlist.add_youtube_url_entry(song_url, channel=channel, author=author)
    ...
    async def cmd_stream(self, player, channel, author, permissions, song_url):
        ...
        return Response(self.str.get('cmd-stream-success', "Streaming."))
    ...
    async def cmd_search(self, message, player, channel, author, permissions, leftover_args):
        ...
        if not info:
            return Response(self.str.get('cmd-search-none', "No videos found."))
        ...
        for e in info['entries']:
            ...
            if str(reaction.emoji) == '\u2705':  # check
                ...
                return Response(self.str.get('cmd-search-accept', "Alright, coming right up!"))
        ...
        return Response(self.str.get('cmd-search-decline', "Oh well :("))
    ...
    async def cmd_resume(self, player):
        ...
        if player.is_paused:
            player.resume()
            return Response(self.str.get('cmd-resume-reply', 'Resumed music in `{0.name}`').format(player.voice_client.channel))
    ...
    async def cmd_shuffle(self, channel, player):
        ...
        return Response(self.str.get('cmd-shuffle-reply', "Shuffled `{0}`'s queue.").format(player.voice_client.channel.guild))
    ...
    async def cmd_clear(self, player, author):
        ...
        return Response(self.str.get('cmd-clear-reply', "Cleared `{0}`'s queue").format(player.voice_client.channel.guild))
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel, param=''):
        ...
        if (param.lower() in ['force', 'f']) or self.config.legacy_skip:
            if permissions.instaskip \
                ...
                await self._manual_delete_check(message)
                return Response(self.str.get('cmd-skip-force', 'Force skipped `{}`.').format(current_entry.title), reply=True)
            else:
    ...
    async def cmd_volume(self, message, player, new_volume=None):
        ...
        if not new_volume:
            return Response(self.str.get('cmd-volume-current', 'Current volume: `%s%%`') % int(player.volume * 100), reply=True)
        ...
        if 0 < new_volume <= 100:
            ...
            return Response(self.str.get('cmd-volume-reply', 'Updated volume from **%d** to **%d**') % (old_volume, new_volume), reply=True)
    ...
    async def cmd_option(self, player, option, value):
        ...
        Valid options:
            save_videos, now_playing_mentions, auto_pause,
        ...
        generic = ['save_videos', 'now_playing_mentions',
        ...
        if player.is_playing:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append(self.str.get('cmd-queue-playing-author', "Currently playing: `{0}` added by `{1}` {2}\n").format(
                    get_entry_title(player.current_entry), player.current_entry.meta['author'].name, prog_str))
            else:
                lines.append(self.str.get('cmd-queue-playing-noauthor', "Currently playing: `{0}` {1}\n").format(get_entry_title(player.current_entry), prog_str))


        for i, item in enumerate(player.playlist, 1):
            if item.meta.get('channel', False) and item.meta.get('author', False):
                nextline = self.str.get('cmd-queue-entry-author', '{0} -- `{1}` by `{2}`').format(i, get_entry_title(item), item.meta['author'].name).strip()
            else:
                nextline = self.str.get('cmd-queue-entry-noauthor', '{0} -- `{1}`').format(i, get_entry_title(item.title)).strip()
    ...
    async def cmd_clean(self, message, channel, guild, author, search_range=50):
        ...
        try:
            ...
            return Response(self.str.get('cmd-clean-invalid', "Invalid parameter. Please provide a number of messages to search."), reply=True)
        ...
        if self.user.bot:
            if channel.permissions_for(guild.me).manage_messages:
                ...
                return Response(self.str.get('cmd-clean-reply', 'Cleaned up {0} message{1}.').format(len(deleted), 's' * bool(deleted)))
    ...
    async def cmd_pldump(self, channel, author, song_url):
        ...
        return Response("Sent a message with a playlist file.")
    ...
    async def cmd_roles(self):
        ...
        Lists all roles availble. Different roles have different permissions to manage this bots.
        """
        return Response("Available roles: " + ", ".join([group.name for group in self.permissions.groups]))
    ...
    async def cmd_listids(self, guild, author, leftover_args, cat='all'):
        ...
        return Response("Sent a message with a list of IDs.")
        ...
        if not user_mentions and target:
            ...
            if user == None:
                try:
                    ...
                    return Response("Invalid user ID or server nickname, please double check all typing and try again.", reply=False)
    ...
    async def cmd_setname(self, leftover_args, name):
        ...
        return Response("Set the bot's username to **{0}**".format(name))
    ...
    async def cmd_setnick(self, guild, channel, leftover_args, nick):
        ...
        return Response("Set the bot's nickname to `{0}`".format(nick))
    ...
    async def cmd_setavatar(self, message, url=None):
        ...
        return Response("Changed the bot's avatar.")
    ...
    async def cmd_disconnect(self, guild):
        ...
        return Response("Disconnected from `{0.name}`".format(guild))
